...
export async function GET(req: Request) {
	...
	try {
		...
		const doc = await collection.findOne<{ userId: ObjectId; displayName?: string; age?: number; bio?: string }>(
			{ userId: targetId },
			{ projection: { _id: 0, userId: 0, displayName: 1, age: 1, bio: 1 } }
		)
		...
		return NextResponse.json({ displayName, age, bio }, { headers })
	} catch {
